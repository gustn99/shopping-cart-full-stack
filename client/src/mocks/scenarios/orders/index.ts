...
export const ordersScenarios = {
  postSuccess: http.post("/api/orders", () => {
    return HttpResponse.json({ orderId: 100 }, { status: 201 });
  }),
  ...
  getSuccess: http.get("/api/orders/:orderId", () => {
    return HttpResponse.json({ products: [], coupons: [], isRemoteArea: false, deliveryFee: 3000 }, { status: 200 });
  }),
  ...
  patchSuccess: http.patch("/api/orders/:orderId", () => {
    return HttpResponse.json({ couponId: [1], isRemoteArea: true, deliveryFee: 6000 }, { status: 200 });
  }),

  patchCouponExpired: http.patch("/api/orders/:orderId", () => {
    return HttpResponse.json({ errorCode: "COUPON_EXPIRED", errorMessage: "만료된 쿠폰입니다." }, { status: 422 });
  }),

  getDiscountSuccess: http.get("/api/orders/:orderId/discount", () => {
    return HttpResponse.json({ discountAmount: 5000 }, { status: 200 });
  }),
